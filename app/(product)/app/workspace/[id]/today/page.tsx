...
// "Today" — the J1 landing ("What needs me right now?"). This is the former
// Overview surface, promoted to its customer-vocab home under /today as part
// of the 5-tab IA (workspace-ia-simplification-2026-06-14). The workspace
// index `/` redirects here so the default landing highlights the Today tab.
//
// Thin server loader: all presentation, copy, and state logic lives in
// `overview-view.tsx` (DB-free, unit tested). This file only fetches and
// shapes data, then hands it over.

export default async function WorkspaceTodayPage({ params }: PageProps) {
  ...
  const [pendingApprovals, openFlags, recentHandoffs, workspace, onboarding, activePause] =
    await Promise.all([
      ...
      // pause (if any) so the surface can render "Plaino is paused"
