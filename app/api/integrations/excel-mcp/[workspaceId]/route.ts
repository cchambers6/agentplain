/**
 * app/api/integrations/excel-mcp/[workspaceId]/route.ts
 *
 * Workspace-scoped HTTP entry to the Excel MCP server. Mirrors the
 * Outlook MCP route. Auth + envelope handling only — tool invocations
 * delegate to `lib/integrations/excel-mcp/json-rpc.ts:dispatch`.
 */
...
export async function POST(req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  ...
  const server = buildExcelMcpServer({ workspaceId });
...
function responseHttpStatus(response: JsonRpcResponse<unknown>): number {
  ...
  if (data?.code) return excelErrorCodeToHttpStatus(data.code);
  switch (code) {
