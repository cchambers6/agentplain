/**
 * app/api/integrations/google-drive-mcp/[workspaceId]/route.ts
 *
 * Workspace-scoped HTTP entry to the Google Drive MCP server. Thin wrapper
 * over `lib/integrations/mcp-core/route.ts` — auth + envelope handling live
 * there; this file only binds the Drive server factory + tool registry.
 */

import { type NextRequest, NextResponse } from 'next/server';
import { handleMcpGet, handleMcpPost, type McpRouteSpec } from '@/lib/integrations/mcp-core';
import {
  buildDriveMcpServer,
  DRIVE_NAMESPACE,
  DRIVE_TOOLS,
  type DriveMcpServer,
} from '@/lib/integrations/google-drive-mcp';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const SPEC: McpRouteSpec<DriveMcpServer> = {
  buildServer: buildDriveMcpServer,
  tools: DRIVE_TOOLS,
  namespace: DRIVE_NAMESPACE,
};

interface RouteContext {
  params: Promise<{ workspaceId: string }>;
}

export async function POST(req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  const { workspaceId } = await ctx.params;
  return handleMcpPost(req, workspaceId, SPEC);
}

export async function GET(req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  const { workspaceId } = await ctx.params;
  return handleMcpGet(req, workspaceId, SPEC);
}
