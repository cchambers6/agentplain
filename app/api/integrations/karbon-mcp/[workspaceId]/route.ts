/**
 * app/api/integrations/karbon-mcp/[workspaceId]/route.ts
 *
 * Workspace-scoped HTTP entry to the Karbon MCP server. Thin wrapper
 * over `lib/integrations/mcp-core/route.ts` — auth + envelope handling
 * live there.
 */

import { type NextRequest, NextResponse } from 'next/server';
import {
  handleMcpGet,
  handleMcpPost,
  type McpRouteSpec,
} from '@/lib/integrations/mcp-core';
import {
  buildKarbonMcpServer,
  KARBON_NAMESPACE,
  KARBON_TOOLS,
  type KarbonMcpServer,
} from '@/lib/integrations/karbon-mcp';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const SPEC: McpRouteSpec<KarbonMcpServer> = {
  buildServer: buildKarbonMcpServer,
  tools: KARBON_TOOLS,
  namespace: KARBON_NAMESPACE,
};

interface RouteContext {
  params: Promise<{ workspaceId: string }>;
}

export async function POST(req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  const { workspaceId } = await ctx.params;
  return handleMcpPost(req, workspaceId, SPEC);
}

export async function GET(req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  const { workspaceId } = await ctx.params;
  return handleMcpGet(req, workspaceId, SPEC);
}
