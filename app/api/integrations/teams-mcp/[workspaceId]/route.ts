/**
 * app/api/integrations/teams-mcp/[workspaceId]/route.ts
 *
 * Workspace-scoped HTTP entry to the Teams MCP server. Mirrors
 * `app/api/integrations/outlook-mcp/[workspaceId]/route.ts` one-for-one:
 * the route is the outer protocol seam — auth + envelope handling — and
 * delegates every tool invocation to
 * `lib/integrations/teams-mcp/json-rpc.ts:dispatch`. The handler does
 * NOT speak Microsoft Graph; only the inner server does.
 *
 * Auth model is identical to outlook-mcp's: `x-agentplain-mcp-key` for
 * fleet code, session+workspace-membership fall-through for browsers.
 */
...
} from '@/lib/integrations/teams-mcp';
...
export async function POST(
  ...
  const server = buildTeamsMcpServer({ workspaceId });
...
function responseHttpStatus(response: JsonRpcResponse<unknown>): number {
  ...
  if (data?.code) return teamsErrorCodeToHttpStatus(data.code);
  switch (code) {
