...
| 2026-08-26 | branch `launch/narrow-to-law` | Same machine-only memory ceiling. Reproduced on an **unmodified** checkout first: the very first push of this branch, still pointing at `4bc42b5` with zero changes, OOM'd in `next build` at the default heap — so the ceiling is environmental, not a regression from this branch. Standalone `tsc --noEmit` then OOM'd at `--max-old-space-size` of 6144 **and** 9216 on the same 16 GB machine, so a local typecheck was not obtainable either. Compensating evidence run instead: `npm run check:claims` (27/27 pass), `npm run test:gate` (5,916 tests, 5,895 pass, 0 fail, 21 quarantined skips, exit 0), and `next lint` scoped to all 11 changed files (clean). | Verify the branch's Vercel preview build goes green on the PR — that is the only typecheck of this change that exists. Local repro still tracked under the build-gate memory-ceiling follow-up. |
| — | — | — | — |
