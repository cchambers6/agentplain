...
| 2026-08-30 | branch `ops/outcome-owners-tenancy-verification` | `build:no-migrate` OOMs locally again — third occurrence of the 2026-08-02 class. Two runs, both `FATAL ERROR: Reached heap limit` during `Linting and checking validity of types`: (1) Node default heap, died at ~4.1 GB after 28 min (last GC `Mark-Compact 4089.1 -> 4087.2 MB`); (2) `--max-old-space-size=6144`, the ceiling `.github/workflows/tests.yml` uses — same phase, same error. Host had ~3.8 GB of 16 GB physically free, so it is bound by available RAM, not by the V8 ceiling. **Correction to the 2026-08-02 row:** this is not "OOMs at 8 GB" — 4 GB, 6 GB, 8 GB and 12 GB have now each failed. The gate fails at whichever ceiling is reached first given free RAM. Diff is additive-only: 7 new files under `lib/`, nothing in `app/` imports them. | Verified in place of the gate: all other pre-push layers passed (lint clean, brand-gate 0 new, voice-gate 0 new, connector-dispatch 15/15, `check:claims` 27/27); `tsc --noEmit` clean over `lib/tenancy/**` + `lib/verification/**` (config proved non-blind with a planted type error); 36/36 new tests pass. Confirm the PR's `tests` workflow and Vercel preview both go green. Consider retiring the local build gate now that CI runs the full suite per-PR (wired 2026-08-11) — a 28-minute local gate that ends in OOM drives people to `HUSKY=0`, which disables every layer including the sub-second claim checks. |
| — | — | — | — |
