...
**Rev 4 (2026-08-11):** adds root cause 9 (the queue has no notion of value — measured by the Librarian auditing its own action queue) + mechanism M9 + EU-14/EU-15, and **amends M2, which as specified in revs 1–3 was insufficient**: it surfaced pending items faithfully but in ledger order under one flat 7-day SLA, which on 2026-08-11's real data would have escalated a scratch-file sweep on identical terms with a revenue-blocking production outage. Rev 1 assumed surfacing was the hard part. It was not — ordering is. A reader that faithfully automates the wrong order has not solved anything.

---

## 0. What actually happened, compressed to its mechanism

Nine root causes — seven from the 2026-08-09 audit cycle, one observed live during this spec's own execution, one measured 2026-08-11 by the Librarian auditing its own action queue:
...
9. **The queue has no notion of value.** Measured by the Librarian against its own action queue, 2026-08-11 (`feedback_action_queue_ranks_self_maintenance_over_business_2026_08_11`, live store): the production outage — nothing built since 2026-06-17 has reached a customer — sat at rank 5 of 5, beneath "approve a scratch-file sweep," for three consecutive passes (#205–#207); and six pending business items appeared in none of the six recorded queue blocks, including a GitHub PAT leaked 2026-06-09 and **still unrevoked** — 14 items pending in the ledger, 8 surfaced, and nothing compared the two counts. The structural fault is arithmetic, not negligence: the Librarian generates queue items about itself as a by-product of self-audit at its own cadence (96 runs/day), while business items arrive at a rate it does not control (zero external arrivals across 36 consecutive drains) — so any ranking that weights recency of attention converges on self-generated items monotonically, with no single pass doing anything wrong. **The producer of the ranking was also the producer of most of the items** — root cause 4's self-reporting disease, one level up. Declared priority cannot fix it: the same ledger declares the leaked PAT `priority: 6` of 8, carries three items with `priority: None` (which any priority-keyed ordering silently drops), one with `medium`, and two using `severity` instead — a field that unreliable at both ends is evidence, not input. This is the other eight compressed: detection works, recording works, self-correction now works (the Librarian caught four of its own errors in three hours by checking claims against files); *ranking* does not — so the loudest item gets worked and the most valuable item waits. It directly undermines M2 as specified in revs 1–3, which is why rev 4 amends it.
...
**Ship-day falsification rule:** several of these checks MUST fire immediately on today's real data (11 stale queue items, 8 stale green PRs). A prevention layer that ships green on today's repo is broken by construction. The acceptance tests in §5 encode this. *Rev 4 extends the rule to queue order:* against the 14 items pending as measured 2026-08-11, the ranked output MUST put the production outage at the head of the business lane — behind only the leaked-PAT exposure item, which the derived order correctly places above it — and MUST put the scratch-file sweep in the fleet lane, never in a business position.
  ...
  └─ writes ≥1 status line per run  ──►  per-task ledger (agent store)
       └─ read by the contracts checker   (on-host, watchdog session;
          every 30 min                     failure domain: host)
            └─ findings + host census + ranked queue mirror + git state
...
## 2. The nine mechanisms

Naming: **M1 Pulse**, **M2 Reader**, **M3 Earned Green**, **M4 Evidence Discipline**, **M5 Landing Alarm**, **M6 Headroom Ratchet**, **M7 Doctrine Expiry**, **M8 Artifact Truth**, **M9 Value Order**. Nine mechanisms, but still only **three moving parts** plus one pure function: one on-host publisher (M1's writer + M3 + M6 sampling), one off-host workflow trio sharing one issue-upsert helper (M1's reader + M2 + M5 + M6 thresholds), one evidence toolchain (M4, which M7 extends), and M9's ranker — a script with no schedule and no workflow of its own, invoked by the publisher and judged by the reader. Everything else is configuration in four YAML files.
...
**Rev 4 amendment — M2 as originally specified was insufficient, and here is why.** Everything above solves "written faithfully, read by nobody." Root cause 9 shows the queue also fails *when read*: the mirror preserved ledger order, and the reader applied one flat 7-day SLA to every pending item regardless of content — so it would have opened issues for `memory-dir-scratch-bloat` and for the production outage on identical terms and identical cadence, and Conner's notification stream would have reproduced the Librarian's inverted queue with GitHub branding on it. A reader that faithfully surfaces a scratch-file sweep beside — or above — a revenue-blocking outage has not solved anything; it has automated the wrong order. The rev-1 text assumed surfacing was the hard part. It was not. Three changes, all consuming M9:

1. **The mirror's queue section is replaced by M9's ranked output** (`rank-queue.mjs`, run by the publisher inside the same payload build, same sanitization pass). Every pending item arrives banded and ordered, business lane and fleet lane separated. The reader never sees ledger order at all.
2. **SLAs and bump cadence key on band, not a flat 7 days:** B0 exposure → issue on first sight, no grace period, daily bump; B1 customer/revenue → issue at 48 h, twice-weekly bump; B2 delivery → 7 days (matching M5's threshold); B3 fleet hygiene → 14 days, weekly bump. Issues carry the band as a label, so the notification stream sorts by the same order the queue does.
3. **Total accounting is enforced, not hoped for:** the reader asserts count(ranked items) = count(pending items in the mirror); any shortfall is a `ranking-incomplete` finding → `fleet-ops` issue. The evidence file's rule — *enumerate, don't sample* — becomes a machine check. 14-pending/8-surfaced can no longer happen silently.
...
### M9 — Value Order: rank is derived, admission is a door, the surface is generated

**The honest premise first.** Every mechanism above assumes that once an item is detected, recorded, and surfaced, the right one gets worked. Root cause 9 falsifies that: with detection, recording, and self-correction all demonstrably working, the queue still worked the loudest item and starved the most valuable one for three consecutive passes. The fix cannot be "rank more carefully" — any judgment applied per-pass drifts toward what the judge can see, which is itself; that is how the last three audits' findings rotted and how this queue inverted. So nothing in this mechanism asks any agent to judge value at ranking time. Rank falls out of properties the item already carries or a check can establish cheaply, and the judgment that genuinely cannot be derived — *what counts as customer value* — is concentrated into one small, PR-ratified file instead of being re-exercised 96 times a day by the party with the conflict of interest.

**Mechanism, part 1 — the value map.** `ops/value-map.yaml`: the standing customer-value bar ("would an SMB pay $500/mo for just this?") frozen into a subject registry. Four bands, strictly ordered; each carries match rules over the item's `subject` field (stamped at admission, part 3), with a keyword fallback over id+title+ask for items that predate the door:

- **B0 exposure** — leaked credentials, exposed customer data, active legal exposure. Tripwire patterns (`pat|token|secret|credential|leak|revoke|expose`) — deliberately narrow, M7-style: a tripwire for the pattern that burned us, not NLP.
- **B1 customer/revenue** — the production deploy path, customer-facing domains, billing/Stripe, the design-partner pipeline, legal-entity blockers to contracting: anything that blocks a customer from seeing, using, or paying for the product.
- **B2 delivery** — finished-but-unlanded work (green PRs, unpushed main). M5 remains the *alarm* for these conditions; B2 is where their queue items rank when queued.
- **B3 fleet hygiene** — the fleet's own machinery: memory store, Librarian cadence, scheduler tasks, budgets, WORKING_STATE.

Band beats everything. Within a band, order is **age — oldest `raised` first**: monotone, non-gameable, already present on every item, and honest about the fact that within-band value differences are not derivable (§7.8). Declared `priority`/`severity` fields become display metadata and never order anything — the live ledger declares the leaked PAT `priority: 6` of 8 and carries `None`/`medium`/`severity:` variants; a field that unreliable is evidence for this design, not input to it. An item matching no band ranks **B-UNKNOWN**: rendered at the head of the fleet lane flagged `classify me`, plus a `queue-unclassified` issue — unknown is not important, but it is never silently bottom, and classifying it costs a one-line value-map PR.

**Mechanism, part 2 — the surface is generated, never composed.** `scripts/ops/rank-queue.mjs rank` reads the live queue (path via EU-2's resolver), classifies every `status: pending` item, and emits the complete ranked output: **business lane** (B0–B2) and **fleet lane** (B3), separated, with the self-generated-vs-external item ratio printed in the header (the evidence file's rule 3, made structural). The Librarian's `WORKING_STATE.md` action-queue block and M2's mirror both consume this output **verbatim** — the Librarian renders it; it no longer composes it. This is the structural fix for 14-pending/8-surfaced: an agent hand-composing a view samples what it happened to touch; a script enumerates. It also removes the conflict of interest at its root — the component that produces most of the items no longer produces the ranking.

**Two lanes, not one order — self-maintenance is floored, not starved.** The 2026-08-11 failure was ordering, not existence: self-maintenance work produced most of that night's real findings, and a mechanism that strict-globally-orders it below business work starves the very loop that caught four errors in three hours. So B3 is not the bottom of one list — it is a **separate lane, always rendered**: its head item appears in every surface (WORKING_STATE block, brief, mirror) and gets M2 issues on its own 14-day SLA. A scratch sweep can never sit above the outage because they are never in the same list; a business drought can never starve the fleet lane because the lanes do not compete for slots. One deliberate consequence, accepted: a genuinely urgent *fleet* emergency never outranks business items in the queue view — correct, because host emergencies already have their own non-queue path (M1/M6 fire in ≤2 h; the queue is for human decisions, not alarms).

**Mechanism, part 3 — admission through one door.** The six un-surfaced items are the same disease at the membership layer: an item that never reaches the ranked surface cannot be ranked at all. Two guards, the provenance-write-door pattern applied to the queue:

1. **One admission helper.** `rank-queue.mjs admit` becomes the only legal way to add a queue item: it stamps `subject` (what artifact/system the item is about — the field part 1 classifies on), `producer` (which session or task filed it — the field the self-vs-external ratio counts on), and `raised`, and validates against **one** schema. The dual `title/detail/priority` vs `ask/evidence/severity` shape is retired: three live items carry `priority: None` precisely because two schemas let orderings key on a field a third of items lack — a dual schema is a warning sign, not a nit.
2. **Membership reconciliation.** The contracts checker (M3's script, same invocation) diffs escalation-shaped facts against queue membership: `status: pending` memory files in the live store, open `fleet-ops`/`dispatch-overdue`/`delivery-stalled` issues, M5 conditions — any such fact with no corresponding queue entry → `unadmitted-item` finding → issue. Together with M2's count assertion (which covers the ledger→surface half), both layers of "never entered the queue" now fire a check instead of waiting for an audit to happen to notice.

**Where it runs and why.** The ranker and value map live in-repo — value judgments change on the record, in a PR, not in any session's per-pass discretion. The ranker executes on-host inside the publisher (the queue lives there); its output is judged off-host by M2's reader like every other payload section. No agent consults the value map at read time — sessions consume the ranked output and never re-derive it, for the same reason M4 forbids re-deriving execution counts.

**Trigger.** Every heartbeat publish (ranked mirror); every watchdog run (membership reconciliation); CI on PRs touching the ranker, its fixtures, or the value map.

**Consumer.** Conner, via M2's band-labeled issues and the brief (which renders the business-lane head, then the fleet-lane head, in that order); resuming sessions, via the generated WORKING_STATE block — the charter's canonical first read now shows the derived order by construction. Consumption receipt: unchanged from M2 — `status` leaves `pending`, the next mirror reflects it, the issue closes.

**Failure mode and what watches it.** The value map can rot as the product changes — caught the M7 way: band definitions carry `verified`/`expires` (180-day default; value definitions are more stable than capability probes) and an expired definition is a `constraint-expired`-family finding. The keyword fallback can misclassify legacy prose — bounded by EU-14's one-time `subject` backfill of all 14 pending items (mechanical, PR-reviewed), after which the fallback only handles items that bypassed the door, which is itself an `unadmitted-item` finding. The ranker rots like any script — fixtures in CI, and M2's count assertion catches a ranker that starts dropping items. A session could still hand-edit the WORKING_STATE block over the generated output — the block's first line is a generation stamp (`rank-queue.mjs @ <ts> over <n> pending`, EU-15), and a stampless or count-wrong block is exactly what the reconciliation flags.

**Deliberate-failure test.** The fixture is **a frozen, sanitized snapshot of the live queue as measured 2026-08-11** — real data, not synthetic. `node --test scripts/ops/rank-queue.test.mjs` MUST assert: `revoke-flatsbo-pat` classifies B0 and heads the business lane *despite* its declared `priority: 6` — derived-beats-declared, tested on the item that proves the need; `prod-deploy-dead-since-06-17` heads B1, above every hygiene item; `memory-dir-scratch-bloat` and `memory-dangling-links-class-a` land in the fleet lane and hold no business position; all 14 pending items appear exactly once across the two lanes; an item with `priority: None` ranks normally; deleting one item's queue entry while its pending memory file remains yields exactly one `unadmitted-item` finding. Ship-day live run: `rank-queue.mjs rank` over the real queue MUST put the production outage at business rank ≤2 (behind only the exposure item) with the scratch sweep in the fleet lane, and M2's first banded run MUST open a B0 issue for the leaked PAT the same day. **A first run in which the outage does not head the post-exposure business lane, or in which the two-months-leaked PAT opens no same-day issue, is a failed acceptance test.**
...
| Queue bands | B0 exposure > B1 customer/revenue > B2 delivery, in one **business lane**; B3 fleet hygiene in a separate, always-rendered **fleet lane**. Within a band: age, oldest `raised` first. Declared `priority`/`severity` never order anything. No-match → B-UNKNOWN at the head of the fleet lane + `queue-unclassified` issue. |
| Band SLAs (supersede M2's flat 7 d) | B0: issue on first sight, daily bump. B1: 48 h, twice-weekly bump. B2: 7 d (=M5). B3: 14 d, weekly bump. Band rides the issue as a label. |
| Queue schema | One shape, stamped at admission via `rank-queue.mjs admit` (`subject`, `producer`, `raised`); the dual `title/priority` vs `ask/severity` shape is retired. Value-map band definitions carry `verified`/`expires` (180-day default) like M7 constraints. |
...
Wave C (parallel):  EU-4 (←EU-3)   EU-6 (←EU-1,5)  EU-7 (←EU-5)   EU-8 (←EU-5,6)   EU-11 (←EU-3,9)   EU-14 (←EU-2)
Wave D (parallel):  EU-10 (←EU-5,6,7)   EU-13 (←EU-1,6 — extends EU-6's script)   EU-15 (←EU-6,14 — extends EU-1/3/6's scripts)
...
### EU-14 · Value map + queue ranker + live-snapshot fixtures — `M` (Wave C)
**Deliverable.** `ops/value-map.yaml` (four band definitions with match rules and `verified`/`expires`, per §M9) + `scripts/ops/rank-queue.mjs` (`rank` and `admit` subcommands; single queue-item schema; reads the live queue via EU-2's resolver) + `scripts/ops/rank-queue.test.mjs` whose fixture is the frozen, sanitized 2026-08-11 live-queue snapshot + a one-time `subject`/`producer` backfill of the 14 pending items in the live `data/conner-queue.yaml` (mechanical, derived from each item's existing text; the backfill diff ships in the same PR so it is reviewed).
**Files.** New: `ops/value-map.yaml`, `scripts/ops/rank-queue.mjs`, `scripts/ops/rank-queue.test.mjs` + fixture. Edit: live `data/conner-queue.yaml` (backfill — via the resolver, live store only, never the inert twin).
**Acceptance (runnable).** `node --test scripts/ops/rank-queue.test.mjs` → all §M9 fixture assertions pass (PAT = B0 head despite `priority: 6`; outage heads B1; both memory-hygiene items fleet-lane only; 14/14 accounted exactly once; `priority: None` ranks normally; missing-entry fixture yields one `unadmitted-item`). Live: `node scripts/ops/rank-queue.mjs rank` → business lane head is `revoke-flatsbo-pat`, `prod-deploy-dead-since-06-17` at business rank ≤2, scratch sweep in the fleet lane, header ratio populated, output count = ledger pending count.
**Depends on:** EU-2. **Parallel:** with EU-4/6/7/8/11.

### EU-15 · Banded reader + admission reconciliation + generated surface — `M` (Wave D)
**Deliverable.** M2's rev-4 amendments in `scripts/ops/escalate-decisions.mjs` (consume the ranked mirror; band-keyed SLAs, bumps, and labels per §3; `ranking-incomplete` count assertion) + ranked-mirror emission in `publish-heartbeat.mjs` (calls `rank-queue.mjs rank`, replaces the raw queue mirror, same sanitization and secret-scan pass) + `unadmitted-item` membership reconciliation in `check-task-freshness.mjs` (diff pending-status memory files + open fleet-ops-family issues + M5 conditions against queue membership) + Librarian charter/prompt amendment: the `WORKING_STATE.md` action-queue block is rendered verbatim from `rank-queue.mjs rank` output with the generation stamp as its first line — consumed, never composed; new items enter only via `rank-queue.mjs admit`.
**Files.** Edit: `scripts/ops/escalate-decisions.mjs` + its test file, `scripts/ops/publish-heartbeat.mjs`, `scripts/ops/check-task-freshness.mjs`, `LIBRARIAN_CHARTER.md` + the Librarian task's SKILL.md under `C:\Users\conne\Claude\Scheduled\`.
**Acceptance (runnable).** `node --test scripts/ops/escalate-decisions.test.mjs` → new cases pass: fixture B0 item pending 0 days yields `[create]` (no grace); fixture B3 item pending 10 days yields `[]` (14-day SLA not reached); ranked mirror carrying 13 of 14 pending items yields exactly one `ranking-incomplete` action. Live: the first banded run opens a B0 issue for `revoke-flatsbo-pat` the same day (**zero B0 issues on first run = failed acceptance**); the next Librarian pass's WORKING_STATE block begins with the generation stamp and accounts for every pending item across the two lanes.
**Depends on:** EU-6, EU-14 (extends EU-1's and EU-3's scripts). **Parallel:** with EU-10/13.
...
Claiming total prevention is the same overreach that produced the audit's false findings. These eight are not prevented; each is made survivable in a specific, testable way:
...
8. **Within-band value judgment is irreducibly human, and the value map is frozen human judgment.** M9 derives *which band* an item belongs to and orders within bands by age; it cannot rank two B1 items against each other by true value, and the value map's contents — what counts as customer-facing — are a human call. The design does not pretend otherwise; it minimizes the touchpoint: the judgment is made once, in a ratified PR, instead of 96 times a day inside the loop where root cause 9 lives, and thereafter costs one line of PR per novel subject (prompted by a `queue-unclassified` issue, so the need is loud). Survivable because the bands are few and coarse — misjudging *within* B1 costs little when everything in B1 structurally outranks all hygiene work — and because B-UNKNOWN makes novel subjects loud instead of silently bottom. Residuals, accepted and named: an item whose own text misdescribes its subject is misclassified until a probe or audit touches it; B0's keyword tripwire shares M7's stated phrase-list limits; and age-within-band is a proxy, not value — when two revenue blockers genuinely compete, a human picks, and no mechanism here claims otherwise.

---

*Built on `origin/main` @ `5606114`. Sources: PR #397 (with corrections banner), PR #398, the live agent-store memory (`feedback_dual_memory_store_trap`, `project_audit_2026_08_09_retractions`, `project_scheduler_total_execution_outage_2026_08_09`, `conner-queue.yaml` — live tree verified by tell: 358 `[PROCESSED]` markers vs 0–1 in the inert twin). Rev 4 additionally: `feedback_action_queue_ranks_self_maintenance_over_business_2026_08_11` and the live `data/conner-queue.yaml` as of 2026-08-11 (14 pending items; declared-priority anomalies quoted verbatim; live tree re-verified by tell — 53 `[PROCESSED]` markers post the 2026-08-10 marker-archive, vs 1 in the inert twin). Every number herein is read from those artifacts, not derived.*
