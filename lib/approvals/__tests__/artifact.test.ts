...
/**
 * Card chrome is no longer matched by pattern anywhere in production -- it is
 * a named field (`RenderedApproval.chrome`) that the artifact simply does not
 * read, so `isCardChromeBlock` / `CARD_CHROME_PATTERNS` / `CARD_CHROME_KINDS`
 * are gone.
 *
 * This TEST still needs to recognise a chrome sentence in order to assert one
 * never reaches an artifact. It does so by asking the PRODUCER what its
 * chrome sentences are -- rendering every kind and collecting the `chrome`
 * field -- and then comparing whole lines for equality. Producer-anchored and
 * exact: it cannot drift from the renderer, and it cannot match a sentence a
 * customer merely started the same way.
 */
const RENDERER_CHROME_SENTENCES: ReadonlySet<string> = new Set(
  (Object.keys(WorkApprovalKind) as Array<keyof typeof WorkApprovalKind>).flatMap(
    (k) => renderApprovalPayload(WorkApprovalKind[k], {}).chrome ?? [],
  ),
);

function looksLikeCardChrome(block: string): boolean {
  return RENDERER_CHROME_SENTENCES.has(block.trim());
}
...
test("every placeholder the RENDERER emits for an empty payload is recognised here", () => {
  ...
  const probe = (kindName: string, rendered: { body: string[] }) => {
    for (const raw of rendered.body) {
      ...
      // Card chrome used to be skipped here via isCardChromeBlock(). It no
      // longer needs skipping: chrome is a separate field on
      // RenderedApproval and never appears in `body` at all. If a render
      // site regresses and puts it back, this probe now REPORTS it rather
      // than silently tolerating it -- which is a strictly stronger check.
...
test("no kind loses a body paragraph merely for opening with the chrome's first words", () => {
  ...
  // There is no predicate left to gate. The opener is customer prose sitting
  // in `body`; chrome lives in `chrome`; nothing compares the two. Asserted
  // here as a property of the DATA rather than of a matcher, because the
  // matcher is what kept getting this wrong.
  assert.equal(looksLikeCardChrome(opener), false);
});
...
test("the recording-consent artifact keeps the terms and drops the pending promise", () => {
  ...
  assert.ok(
    (rendered.chrome ?? []).includes("Recording stays off until you approve it here."),
    "the fix removed the sentence from the CARD, where it is true and useful",
  );
  // ...and it is NOT in body, which is what makes the artifact drop it
  // without inspecting anything.
  assert.equal(
    rendered.body.includes("Recording stays off until you approve it here."),
    false,
    "chrome must live in `chrome`, not in `body`",
  );
});
...
test("a portal message keeps a line that is chrome only on the CONSENT card", () => {
  ...
  // The card's own promise is still produced -- in its own field, where a
  // post-approval consumer cannot confuse it for the customer's second
  // paragraph, which reads identically to the consent card's chrome.
  assert.deepEqual(rendered.chrome, [
  ...
  // THE POINT, restated structurally. The consent card's chrome sentence and
  // this customer's second paragraph are the SAME STRING. Under the old
  // per-kind predicate that collision was survivable only because the keying
  // happened to be right; under field separation the collision is irrelevant,
  // because the artifact reads `body` and the sentence's chrome twin lives in
  // a different field on a different kind. Nothing has to be keyed correctly.
  const consentLine = "Recording stays off until you approve it here.";
  assert.equal(
    (renderApprovalPayload(WorkApprovalKind.VOICE_RECORDING_CONSENT, {}).chrome ?? [])
      .includes(consentLine),
    true,
    "the consent card still emits it as chrome",
  );
  assert.equal(
    rendered.body.includes(consentLine),
    true,
    "and the identical sentence is plain body text on a portal message",
  );
});

test("every closed-loop kind emits chrome, and none of it reaches its artifact", () => {
  // The old version of this test checked that a hand-maintained PATTERN table
  // was keyed consistently with CLOSED_LOOP_KINDS. That table is gone. What
  // has to stay true is simpler and stronger: every closed-loop kind emits
  // its pending-state promise in `chrome`, and no artifact ever contains it.
  let examined = 0;

  for (const kind of CLOSED_LOOP_KINDS) {
    examined += 1;
    const enumKind = WorkApprovalKind[kind as keyof typeof WorkApprovalKind];
    const rendered = renderApprovalPayload(enumKind, FIXTURES[kind]!.payload);

    assert.ok(
      rendered.chrome && rendered.chrome.length > 0,
      `${kind} is closed-loop and emits no chrome, so its card no longer tells ` +
        `the owner that nothing has happened yet`,
    );

    const artifact = buildApprovalArtifact(enumKind, rendered);
    for (const line of rendered.chrome!) {
      assert.equal(
  ...
  // "found nothing" must be distinguishable from "examined nothing".
  assert.equal(examined, 5, `examined ${examined} of 5 closed-loop kinds`);
});
