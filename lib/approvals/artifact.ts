...
// -- Card chrome: now dropped BY IDENTITY, not by pattern ----------------
//
// The five closed-loop kinds open with a promise about the CARD's state:
// "Awaiting your approval -- Plaino will send this envelope ... Nothing has
// been sent." That is exactly right under a pair of approve/reject buttons
// and false in an artifact the customer holds AFTER approving.
//
// This module used to strip those sentences with a set of per-kind regexes
// applied to `rendered.body`. That approach shipped a HIGH defect -- a
// PORTAL_CLIENT_MESSAGE legitimately opening "Awaiting your approval on the
// revised scope, we are holding the crew until Friday." had that paragraph
// silently deleted out of the copy, the download AND the mailto -- and after
// four rounds of tightening the patterns the work was parked with the ruling
// "no fourth regex tune".
//
// THE MACHINERY IS GONE. It was not tuned a fifth time; it was made
// unnecessary. `RenderedApproval.chrome` is now a separate field, populated
// at the five render sites that emit these sentences, and `body` carries
// only work product. This module reads `body` and never `chrome`, so chrome
// is excluded by NOT BEING READ.
//
// The security property that follows is the reason it was worth doing: there
// is no longer any code path in which the customer's own sentences are
// inspected to decide whether to keep them. Content loss by mis-match is not
// merely fixed, it is unrepresentable.
//
// What replaces the regex as a CHECK is a producer-anchored assertion in
// lib/approvals/__tests__/chrome-separation.test.ts: for every kind, the
// renderer's `body` must contain none of the known chrome sentences. That
// binds this module to the renderer in the same way RENDERER_FALLBACK_BLOCKS
// already does, and it fails if a future render site puts chrome back into
// `body`.
...
export function buildApprovalArtifact(
  ...
  // The work product itself, and ONLY the work product: `rendered.chrome` is
  // deliberately not read here (see the card-chrome note above), so the
  // pending-state promise cannot reach an artifact that by definition exists
  // only after approval.
  //
  // Two things are still dropped, and both are matched against OUR OWN
  // emitted constants rather than against customer prose:
  //   - spacer lines, because blocks are re-joined with blank lines downstream
  //   - renderer fallbacks ("No body attached."), because a fallback line in
  //     an artifact is an empty artifact wearing a checkmark
  // `isRendererFallbackBlock` is a set-membership test on an exported list
  // that the test suite anchors to the renderer in both directions. It is not
  // a pattern, and it cannot match a sentence the customer wrote unless the
  // customer wrote one of those exact placeholder strings verbatim.
