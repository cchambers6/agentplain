...
export const cpa: VerticalContent = {
  ...
  jtbdTables: [
    {
      ...
      rows: [
        ...
        },
        {
          ...
          withAgentplain:
            "Compliance agent (activating after counsel review) will pre-run the federal + state checklist; until then you review every draft before it's filed",
        },
        {
  ...
  roi: {
    ...
    outputValue: "$10,400 / yr per staff member (sourced)",
    math:
      "Tax season = 80-hour weeks. Doc-chase consumes ~25% of staff hours through 8 weeks (per b2b_vertical_opportunity_analysis_2026-04-27.md §3.4 — \"document chase consumes the front office for 8 weeks a year\"). 0.25 × 80 hours × 8 weeks × $65/hr loaded = $10,400 per staff per season. That sourced figure alone, against the flat ${FLAT}/month ($1,188/yr), is ~9x per staff member, and that is the whole headline. A previously published ~$42k/yr (~35x) figure added ~$31,600/yr for onboarding-letter automation and books-recon reclamation; NEITHER capability exists in SKILL_CATALOG, so roughly 75% of that headline rested on software that was never built. It has been removed rather than relabelled — a hedge word on a sales page is not a substitute for a capability. The sourced $10,400 is the only figure claimed. Because the price is flat, each additional staff member raises the return on an unchanged bill.",
    citation:
      "Pricing per `lib/billing/facts.ts` (`MONTHLY_PRICE_USD_CENTS`) — ONE flat monthly price, the same for every customer and every vertical, whatever the headcount. Trial + money-back mechanics per the same module. Support model per `PARTNER_SUPPORT` in `lib/billing/facts.ts`: priority email/chat and a quarterly async check-in, and explicitly NO reserved human hours. ROI band per `project_pricing_value_anchor.md`. The retired ~$42k/yr headline's onboarding-letter and books-recon components are NOT backed by any `SKILL_CATALOG` entry and have been removed from every claimed figure; only the sourced doc-chase number is claimed. Doc-chase share cited from `b2b_vertical_opportunity_analysis_2026-04-27.md` §3.4. Staff-loading rate is operator-modeled — flagged in capability inbox for primary-research validation.",
    violationAvoidance:
