...
export const general: VerticalContent = {
  ...
  verticalFaq: [
    ...
    },
    {
      ...
      a: `${FLAT} per month. One flat price — the same whether you are a solo operator or a forty-person shop, and the same across every vertical. Month-to-month, with a ${TRIAL_PERIOD_DAYS}-day free trial (card at signup) and a ${MONEY_BACK_GUARANTEE_DAYS}-day money-back guarantee on your first charge. You can cancel anytime.`,
    },
    {
  ...
  roi: {
    multiplier: "26x",
    inputCost:
      `${FLAT}/month flat — ${TRIAL_PERIOD_DAYS}-day free trial, card at signup`,
    outputValue:
      "$3,000 / mo at the conservative end of the value range",
    math:
      `Lower bound of the published value-anchor band ($2,900–$10,600/mo). Universal-admin recovery is the floor case: ~6 owner-hours/week on inbox triage + scheduling + follow-up × $100/hr conservative blended rate × 4.3 weeks = ~$2,580/mo. Against the flat ${FLAT}/month that is ~26x at the floor. Because the price is flat, adding people raises the return rather than the bill. Without a vertical-specific compliance corpus the upside ceiling is lower than the named verticals — that's the honest trade for not having to scope a Custom engagement.`,
    citation:
      "Pricing per `lib/billing/facts.ts` (`MONTHLY_PRICE_USD_CENTS`) — ONE flat monthly price, the same for every customer and every vertical, whatever the headcount. Trial + money-back mechanics per the same module. ROI band per `project_pricing_value_anchor.md`. Universal-admin hour share is an operator-modeled floor — flagged in capability inbox for primary-research validation. `/general` deliberately publishes the low end of the range because the on-ramp scaffolding is lighter than a ratified vertical.",
    violationAvoidance:
