...
export const propertyManagement: VerticalContent = {
  ...
  roi: {
    ...
    citation:
      "Pricing per `project_stripe_both_surfaces.md` (Regular tier per the 2026-05-15 three-tier ratification — Regular is the default entry path; PM operations wanting priority email/chat support and a quarterly async check-in can step up to Partner ($299→$199/seat), and high-intensity multi-state portfolios route to Max (quote-based)). ROI band per `project_pricing_value_anchor.md`. PM-hour distribution and delinquency-float math pending primary-research validation — flagged in capability inbox. Buildium/AppFolio market dynamics referenced from public segment reporting; specific savings claims are operator-modeled, not customer-attested.",
    violationAvoidance:
