...
export const realEstate: VerticalContent = {
  ...
  roi: {
    ...
    citation:
      "Pricing per `project_stripe_both_surfaces.md` (Regular tier per the 2026-05-15 three-tier ratification — Regular is the default entry path; brokerages wanting priority email/chat support and a quarterly async check-in can step up to Partner ($299→$199/seat), and high-intensity multi-office or franchise-scale engagements route to Max (quote-based); trial + money-back mechanics per `lib/billing/facts.ts`, ratified 2026-06-14). Value math per `project_pricing_value_anchor.md` (Regular-tier value range $2,900–$10,600/mo per seat). Realty-specific inputs per `realty_vertical_spec_v1_2026-05-03.md` §1. Coordination-hour ranges per `agentplain_positioning.md` L33. Owner-hour opportunity cost is a 2026-05-08 internal assumption pending primary-research validation — flagged in capability inbox.",
    violationAvoidance:
