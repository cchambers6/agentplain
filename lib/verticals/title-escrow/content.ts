...
export const titleEscrow: VerticalContent = {
  ...
  roi: {
    ...
    citation:
      "Pricing per `project_stripe_both_surfaces.md` (Regular tier per the 2026-05-15 three-tier ratification — Regular is the default entry path; title/escrow operations wanting priority email/chat support and a quarterly async check-in can step up to Partner ($299→$199/seat), and high-intensity multi-state closing engagements route to Max (quote-based)). ROI band per `project_pricing_value_anchor.md` (Regular-tier value $2,900–$10,600/mo per seat). Time-per-file estimates pending primary-research validation (flagged in capability inbox). Closer compensation midpoint based on US BLS 2024 SOC 13-2072, loan officers and related; flagged as operator-modeled.",
    violationAvoidance:
